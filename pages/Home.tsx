...
export const Home: React.FC = () => {
  ...
  const loadData = () => {
    const txs = StorageService.getTransactions().sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  const generateAdvice = async () => {
    ...
    transactions.slice(0, 20).forEach(t => {
      if(t.type === 'expense') {
        const catName = StorageService.getCategories().find(c => c.id === t.categoryId)?.name || 'Unknown';
        ...
        <div className="space-y-3">
          {transactions.length === 0 ? (
            ...
            transactions.slice(0, 5).map((t) => {
              const category = StorageService.getCategories().find(c => c.id === t.categoryId);
              return (
                <div 
                  key={t.id} 
