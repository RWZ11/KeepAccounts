...
export const AddTransaction: React.FC = () => {
  ...
  const categories = StorageService.getCategories().filter(c => c.type === type);
  const accounts = StorageService.getAccounts();

  // Load existing data if in edit mode
  useEffect(() => {
    if (isEditMode && id) {
      const existing = StorageService.getTransactionById(id);
      if (existing) {
  ...
  const handleAiParse = async () => {
    ...
    if (result) {
      ...
      if (result.categoryName) {
        const matched = StorageService.getCategories().find(c => 
